...
public class CartridgeService : BaseItemService<Cartridge, CartridgeRequest, CartridgeResponse>
{
    public CartridgeService(IItemRepository<Cartridge> repository, IMapper mapper, IValidator<CartridgeRequest> validator, IAuditLogService auditLog)
    ...
    public override async Task<Result<byte[]>> ExportItemsToExcel(ItemFilter filter)
    {
        try
        {
            ...
            var worksheet = workbook.Worksheets.Add("Картриджи");
            ...
            worksheet.Cell(1, 5).Value = "Модель принтера";
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                worksheet.Cell(i + 2, 5).Value = items[i].PrinterModel;
